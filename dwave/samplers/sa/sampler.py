...
class SimulatedAnnealingSampler(dimod.Sampler, dimod.Initialized):
    ...
    def sample(self, bqm: dimod.BinaryQuadraticModel,
        ...
        return self._sample(bqm,
                            ...
                            proposal_acceptance_criteria,
                            False)
    def _sample(self, bqm: dimod.BinaryQuadraticModel,
               beta_range: Optional[Union[List[float], Tuple[float, float]]] = None,
               num_reads: Optional[int] = None,
               num_sweeps: Optional[int] = None,
               num_sweeps_per_beta: int = 1,
...
class AnnealedImportanceSampling(SimulatedAnnealingSampler):
    """Annealed importance sampling for binary quadratic models.

    `Annealed importance sampling <https://arxiv.org/abs/physics/9803008>`_ can be used to generate
    weighted samples and estimate normalizing constants for a binary quadratic models. This
    implementation assumes the initial :math:`\\beta`.\ [#]_ is zero. By default each spin is
    updated once in a fixed order per point per :math:`\\beta` according to a Metropolis-Hastings
    update.
    ...
    For more information, see Neal, Radford M. "Annealed importance sampling."
    Statistics and computing 11 (2001): 125-139.


    Examples:
        This example samples from a simple Ising problem.

        >>> from dwave.samplers import AnnealedImportanceSampling
        >>> sampler = AnnealedImportanceSampling()
        >>> h = {'a': 0.0, 'b': 0.0, 'c': 0.0}
        >>> J = {('a', 'b'): 1.0, ('b', 'c'): 1.0, ('a', 'c'): 1.0}
        >>> sampleset = sampler.sample_ising(h, J, num_reads=10)
        >>> print("log_weights" in sampleset.info)
        True
    ...
    See :meth:`.AnnealedImportanceSampling.sample` for a description of the
    parameters.
    ...
    def sample(self, bqm: dimod.BinaryQuadraticModel,
        ...
        Args:
            ...
            num_reads:
                Number of reads. Each read is generated by one run of the
                annealed importance sampling algorithm. If ``num_reads`` is not explicitly
                given, it is selected to match the number of initial states
                given. If initial states are not provided, only one read is
                performed.
            ...
            interrupt_function (function, optional):
                A function called with no parameters between each sample of
                annealed importance sampling. If the function returns True, annealed
                importance sampling terminates and returns with all of the samples,
                energies, and weights found so far.
        ...
        Returns:
            ...
            The `info` field of the sample set contains information about the sampling procedure:
            1. the beta range used,
            2. the beta schedule type used,
            3. timing information (details below), and
            4. log weights and logz estimate
               (see `annealed importance sampling <https://arxiv.org/abs/physics/9803008>`_).
        ...
        Examples:
            This example runs annealed importance sampling on a binary quadratic model
            with various input parameters.
            ...
            >>> # Run with default parameters
            >>> sampleset = sampler.sample(bqm)
            >>> assert "logz_estimate" in sampleset.info
        ...
        if num_sweeps is not None and num_sweeps < 1:
            ...
            raise ValueError(error_msg)
        ss = super()._sample(bqm, beta_range=beta_range,
    ...
    @staticmethod
    def resample(sample_set: dimod.SampleSet, num_resample: int,
                 seed: Optional[int] = None) -> dimod.SampleSet:
        """Resample a sample_set according to their ``log_weights``.

        Args:
            sample_set (dimod.SampleSet): a sample set generated by ``AnnealedImportanceSampling``.
            num_resample (int): Sample size of the resampling procedure.
            seed (Optional[int], optional): Seed to use for the PRNG. Specifying a particular seed
        ...
        Returns:
            dimod.SampleSet: A resampling of ``sample_set`` in proportion to
                ...
                field under the key "indices".
        """
        if len(sample_set) < 1 or num_resample < 1:
            raise ValueError("Sample sizes must be positive (`len(sample_set)`, `num_reads).")

        if "log_weights" not in sample_set.info:
            raise ValueError('"log_weights" is not found in ``sample_set.info``.')

        log_p = sample_set.info['log_weights'] - log_sum_exp(sample_set.info['log_weights'])
        p = np.exp(log_p)
        rng = np.random.default_rng(seed)
        indices = rng.choice(np.arange(sample_set.record.shape[0]), num_resample, p=p)
        # Do not include the original `sample_set.info` because it is no longer relevant
        info = dict(indices=indices.tolist())
        resampled = dimod.SampleSet.from_samples((sample_set.record.sample[indices],
                                                  sample_set.variables),
                                                 sample_set.vartype,
                                                 sample_set.record.energy[indices],
                                                 info)
        return resampled
