...
class SimulatedAnnealingSampler(dimod.Sampler, dimod.Initialized):
    ...
    can be used for heuristic optimization or approximate Boltzmann sampling.
    This implementation approaches the equilibrium distribution by performing
    updates at a sequence of decreasing temperatures, terminating at the target
    :math:`\\beta`.\ [#]_ By default each spin is updated once in a fixed order
    per point per :math:`\\beta` according to a Metropolis-Hastings update.
    When :math:`\\beta` is large the target distribution concentrates, at
    equilibrium, over ground states of the model. Samples are guaranteed to
    match the equilibrium for long, smooth :math:`\\beta` schedules.

    .. [#] :math:`\\beta` represents the inverse temperature,
       :math:`1/(k_B T)`, of a `Boltzmann distribution
       <https://en.wikipedia.org/wiki/Boltzmann_distribution>`_
       where :math:`T` is the thermodynamic temperature in kelvin and
    ...
    def sample(self, bqm: dimod.BinaryQuadraticModel,
               beta_range: Optional[Union[List[float],
               ...
               beta_schedule: Optional[Union[Sequence[float],
                                             np.ndarray]] = None,
        ...
        Args:
            ...
            num_reads:
                ...
                simulated annealing algorithm. If ``num_reads`` is not
                explicitly given, it is selected to match the number of initial
                states given. If initial states are not provided, only one read
                is performed.
            ...
            beta_schedule_type:
                ...
                * "custom"
                ...
                "custom" is recommended for high-performance applications,
                which typically require optimizing :math:`\\beta` schedules
                beyond those of the "linear" and "geometric" options, with
                bounds beyond those provided by default.
                ``num_sweeps_per_beta`` and ``beta_schedule`` fully specify a
                custom schedule.

            beta_schedule:
                Sequence of :math:`\\beta` values swept. Format must be
                compatible with ``numpy.array(beta_schedule, dtype=float)``.
            ...
            initial_states:
                ...
                ``initial_states_generator``. See :func:`~dimod.as_samples` for
                a description of "samples-like".
            ...
            randomize_order:
                When `True`, each spin update selects a variable uniformly at
                random. This method is ergodic, obeys detailed balance and
                preserves symmetries of the model.

                When `False`, updates proceed sequentially through the labeled
                variables on each sweep so that all variables are updated once
                per sweep.
                This method:
            ...
            proposal_acceptance_criteria:
                ...
                When "Metropolis", each spin flip proposal is accepted
        ...
        Returns:
            ...
            sampling, and postprocessing time. All timings are reported in
            ...
            the algorithm spent in sampling states of the binary quadratic
        ...
        # Developer note: the specific keys of the timing dict are chosen to
        #                 be consistent with other samplers' timing dict.
...
def _default_ising_beta_range(h, J,
    ...
    Generally one should optimize min_beta, max_beta and the shape of the
    schedule to maximize some objective such as time to solution.
    The approximations here are simplified, and can be far from optimal in some
    cases. Use custom schedules to address such cases.
    Approximations used are of complexity O(number of biases), O(1)
    approximations are also possible, as are stronger approximations, but
    methods are not bottlenecks in practice.
    We use the maximum bias per spin to give an upper bound on the gap,
    allowing the initial sweeps to be fast mixing.
    We use an approximation to the minimum bias per spin to give a lower bound
    on the minimum energy gap, such that for the final sweeps states are highly
    ...
    # bias = |h_i + sum_j Jij s_j|:
    sum_abs_bias_dict = defaultdict(int, {k: abs(v) for k, v in h.items()})
    if sum_abs_bias_dict:
        min_abs_bias_dict = {k: v for k, v in sum_abs_bias_dict.items()
                             if v != 0}
    else:
        min_abs_bias_dict = {}
    # This loop is slow, but is not a bottleneck for practical implementations
    ...
    if not min_abs_bias_dict:
        # If a flat energy landscape is provided beta_range is ill-defined.

        warn_msg = ('All bqm biases are zero (all energies are zero), this is '
                    'likely a value error. beta_range is irrelevant and set '
                    'arbitrarily to [0.1,1].')
        warnings.warn(warn_msg)
        return([0.1, 1])

    # Selecting betas based on probability of flipping a qubit

    # Hot temperature (lowest beta):
    # We want to be able to quickly search the entire solution space
    # (equilibrate).
    # Scale hot_beta so that all spins are able to flip with probability at
    ...
    # contribute without frustration:
    ...
    # Cold temperatire (largest beta):
    # Towards the end of the annealing schedule, we want to eliminate
    # excitations assuming an optimization application; the samples relax to a
    # local (or ideally global) minima.
    # If we arrive at the ground state (or local minima) on the penultimate
    # iteration, we can require that we have low probability to excite on the
    # final iteration.
    ...
    # case (high connectity and high precision) determining min_delta_energy_i
    # is NP-hard (related to a knapsack problem) we settle for this
    # approximation, which yields satisfactory results in many standard model
    # types.
    ...
    # min_delta_energy is twice the smallest (non-zero) effective field, we
    # approximate this per spin as the smallest associated bias term (h or J).
    if len(min_abs_bias_dict) == 0:
        ...
        if scale_T_with_N:
            ...
            number_min_gaps = 1
        cold_beta = np.log(number_min_gaps/max_single_qubit_excitation_rate) \
